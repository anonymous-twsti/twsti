...
# Jacobi sums
# --------------------------------------------------
# For short Stickelberger relation \theta(a)+\theta(b)-\theta(-c), m|a+b+c,
...
# By definition, this Jacobi sum is also (by definition in [Was97, p.88]):
...
def jacobi_sum_dlog_table(Fq_a):
    ...
    # dlog[k] = (ln_u u^k, ln_u (1-u^k)) = (k, ln_u(1-u^k)) = (k,s) tq u^s = 1 - u^k
