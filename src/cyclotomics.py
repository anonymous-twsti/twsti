...
#
#  /!\   Hypothesis m \ne 2[4] in [Was97, §8.1].
#        The code might not be resilient if given a wrong m'=2m (m odd).
# [Note] Normally, the code is valid for *all* conductors, not only prime powers.
# --------------------------------------------------------------------------------------------------

# Built-in 'K.conductor()' is too general and takes forever, even using 'check_abelian=False'.
...
# because (?) it tries to generically compute max_order() (there is even pbs to *print* the ideal).
# Here we factor K.equation() mod p and create each ideals above p by hand.
# __idx_start control the first prime of the orbit. Probaby not useful.
...
# Applying SageMaths K.complex_conjugation() is way too long (it tries to evaluate at 1/z mod phi ?)
...
def cf_hminus(m):
    ...
    Q   = 1 if m.is_prime_power() else 2;
...
# [BFHP20] Norm relations and computational problems in number fields (Tab.1, seems unconditional ?)
